...
function khach_hang_khoa($ma_kh)
{
    $sql = "UPDATE khach_hang SET trang_thai = '1' WHERE ma_kh=?";
    if (is_array($ma_kh)) {
        foreach ($ma_kh as $ma) {
            pdo_execute($sql, $ma);
        }
...
function khach_hang_go_khoa($ma_kh)
{
    $sql = "UPDATE khach_hang SET trang_thai = '0' WHERE ma_kh=?";
    if (is_array($ma_kh)) {
        foreach ($ma_kh as $ma) {
            pdo_execute($sql, $ma);
        }
